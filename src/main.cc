...
void printNNFromFileUsage() {
  std::cout
    << "usage: fasttext nn_from_file <model> <k> <word_file>\n\n"
    << "  <model>      model filename\n"
...
void nn_from_file(const std::vector<std::string> args) {
  ...
  std::ifstream fin(args[4]);
  if (!fin.is_open()) {
    std::cerr << "word file cannot be opened!" << std::endl;
    exit(EXIT_FAILURE);
  }

  while (std::getline(fin, queryWord)) {
    banSet.clear();
    banSet.insert(queryWord);
    fasttext.getWordVector(queryVec, queryWord);
    fasttext.findNN(wordVectors, queryVec, k, banSet, results);
    for (auto& pair : results) {
      std::cout << queryWord << " " << pair.second << " " << pair.first << std::endl;
    }
  }
  exit(0);
}

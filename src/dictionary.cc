...
Instruction
-------------------
Before training model, it is necessary to initial a vocabulary.
This vocabulary records word, count and type.
  ...
  for (size_t i = 0; i < word.size(); i++) {
    ...
    for (size_t j = i, n = 1; j < word.size() && n <= args_->maxn; n++) {
      ...
      if (n >= args_->minn && !(n == 1 && (i == 0 || j == word.size()))) {
        int32_t h = hash(ngram) % args_->bucket;
        /*
          h is inserted into ngrams in ngrams. To differetiate from
          word, h adds nwords_.
        */
        pushHash(ngrams, h);
      }
...
/*
  ...
  It is worth noting that subwords in entry includes
  wid.

*/
void Dictionary::initNgrams() {
  /*
    In readFromFile, size_ represent number of valid word. 
    Therefore, i can be regarded as wid.
  */
...
Read word from istream and save in word.
...
void Dictionary::initWordCategory(
  ...
  for(auto it_label=terms.cbegin(); it_label!=terms.cend(); ++it_label){
    for(auto it_word=terms.cbegin(); it_word!=terms.cend(); ++it_word){
      if(getType(*it_label) == entry_type::word){
        break;
      }
      if(getType(*it_word) == entry_type::label){
        continue;
      }
      buff[*it_label][*it_word] += 1;
    }
  }
  terms.clear();
  return;
...
void Dictionary::probProportion(std::unordered_map<int32_t, real>& cate2num){
  int32_t cid, wid;
  for(auto it_cid=category_word2prob.begin(); it_cid!=category_word2prob.end(); ++it_cid){
    for(auto it_wid=(it_cid->second).begin(); it_wid!=(it_cid->second).end(); ++it_wid){
      cid = it_cid->first;
      wid = it_wid->first;
      category_word2prob[cid][wid] = (it_wid->second) / cate2num[cid];
    }
  }
  return;
}

void Dictionary::probLog(){
  int32_t cid, wid;
  real prob;
  for(auto it_cid=category_word2prob.begin(); it_cid!=category_word2prob.end(); ++it_cid){
    for(auto it_wid=(it_cid->second).begin(); it_wid!=(it_cid->second).end(); ++it_wid){
      cid = it_cid->first;
      wid = it_wid->first;
      prob = log(it_wid->second) / log(7) / 10;
      if (prob > 0.5) prob = 0.5;
      category_word2prob[cid][wid] = prob;
    }
  }
}

void Dictionary::saveWordCategory(){
  std::ofstream ofs(args_->output + ".dropout");
  for(auto it_cid=category_word2prob.begin(); it_cid!=category_word2prob.end(); ++it_cid){
    for(auto it_wid=(it_cid->second).begin(); it_wid!=(it_cid->second).end(); ++it_wid){
      ofs << words_[it_cid->first].word << "=>";
      ofs << words_[it_wid->first].word << "=>";
      ofs << it_wid->second << std::endl;
    }
  }
  ofs.close();
}
...
*/
void Dictionary::updateWordCategory(dropout_inter_type& buff){
  int32_t cid, wid;
  std::unordered_map<int32_t, real> cate2num;
  // remove redundance
  for(auto it_cate=buff.begin(); it_cate!=buff.end(); ++it_cate){
    for(auto it_word=(it_cate->second).begin(); it_word!=(it_cate->second).end(); ++it_word){
      cid = getId(it_cate->first);
      wid = getId(it_word->first);
      if(cid < 0) break;
      if(wid < 0) continue;
      category_word2prob[cid][wid] = (real)it_word->second;
      cate2num[cid] += (real)it_word->second;
    }
  }
  // count2prob
  // probProportion(cate2num);
  probLog();
  saveWordCategory();
}

void Dictionary::loadVocab(std::set<std::string>& vocab){
  std::string line;
  std::ifstream fvocab(args_->path_vocabulary);
  if (!fvocab.is_open()){
    throw std::invalid_argument("Fail to open vocabulary....");
  }
  while (std::getline(fvocab, line)){
    vocab.insert(line);
  }
  fvocab.close();
}
  ...
  while (readWord(in, word)) {
    ...
    */
    ...
    if (size_ > 0.75 * MAX_VOCAB_SIZE) {
      minThreshold++;
      threshold(minThreshold, minThreshold, vocab);
    }
  }
  threshold(args_->minCount, args_->minCountLabel, vocab);
  initTableDiscard();
  initNgrams();
  if (args_->verbose > 0) {
...
/*
  ...
  -------------------------------
  Process:
  1. Let words_ sort by rule which is "word then label, 
  high frequency then low".
  2. Remove label and word under threshold.
  3. Update word2int_, nwords_ and nlabels_.
...
void Dictionary::threshold(int64_t t, int64_t tl) {
  // words_ 按照先word后label，先频率高 后频率低
...
void Dictionary::threshold(int64_t t, int64_t tl, 
  ...
  // 干掉频率低的tag
  words_.erase(remove_if(words_.begin(), words_.end(), [&](const entry& e) {
        return (e.type == entry_type::word && (e.count < t && vocab.count(e.word) == 0)) ||
               (e.type == entry_type::label && e.count < tl);
      }), words_.end());
  // 释放空间
  words_.shrink_to_fit();
...
/*
  An word can be represented by multiple ways, such as
  wid itself or subword and so on.
  1. A token, in vocabuary, will be replaced by wid itself. When
  you set maxn, subword represent this token.
  2. When a token isn't in vocabulary, a function called "computeSubwords"
  will work.
*/
...
/*
  Read data from file. It is worth noting that pdiscard_ doesn't
...
/*
  ...
  Instruction
  ----------------------
  Category dropout. This function just uses in training process.
...
int32_t Dictionary::getLine(std::istream& in,
  ...
  while (readWord(in, token)) {
    ...
    ntokens++;
    if(type == entry_type::word){
      if(wid < 0 || token == EOS || labels.size()!=1){
        prob = 0;
      }else{
        cate = labels[0] + nwords_;
        auto it_cate = category_word2prob.find(cate);
        if(it_cate == category_word2prob.end()) break;
        auto it_word = (it_cate->second).find(wid);
        prob = it_word->second;
      }
      if(uniform(rng) > prob){
        addSubwords(words, token, wid);
